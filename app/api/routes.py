...
from app.api.schemas import ChatIn, KnowledgeIn, RestaurantCreate
from app.core.config import get_settings
from app.db.models import Conversation, KnowledgeChunk, Reservation, Restaurant
...
async def create_restaurant(
    ...
    r = Restaurant(**body.model_dump()); db.add(r); await db.commit(); await db.refresh(r)
...
async def add_knowledge(
    ...
    x = KnowledgeChunk(restaurant_id=r.id, kind=body.kind, title=body.title, content=body.content, metadata_json=body.metadata, embedding=emb)
    db.add(x); await db.commit(); return {"id": x.id}
